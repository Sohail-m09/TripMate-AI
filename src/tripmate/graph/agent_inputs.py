...
def build_hotel_agent_input(
    ...
    return (
        "Search for accommodation using only "
        "the following trip information.\n"
        f"Destination: {destination or 'Not provided'}\n"
        f"Check-in date: {start_date or 'Not provided'}\n"
        f"Check-out date: {end_date or 'Not provided'}\n"
        f"Adults: {adults if adults is not None else 'Not provided'}\n"
        f"Children: "
...
def build_places_agent_input(
    ...
    return (
        "Find useful places, attractions, or activities "
        "for the following destination.\n"
        f"Destination: {destination or 'Not provided'}\n"
        "Use the places-related intent from this original request:\n"
        f"{state['user_query']}"
    )